        ...
        <div className='grid lg:grid-cols-3 md:grid-cols-2 xxl:gap-10 xl:gap-8 lg:gap-6 gap-4 lg:mt-[65px] mt-10'>
{
    Features_data?.map(({id,title,image},index)=>{
        return(
            <motion.div key={id} className='bg-grad-theme-135 rounded-3xl p-[3px] md:min-h-[444px] min-h-auto' initial={{opacity:0,
                y:80
            }} whileInView={{opacity:1,y:0}} transition={{duration:0.5,ease:"easeIn"}}whileHover={{scale:1.05}}>
                <div className='rounded-3xl bg-jet flex flex-col justify-between items-center h-full'>
                    ...
                    <img src={image} alt="" className={`px-4 ${index===1 && "mb-5"} ${index===2 && "mb-8"}`} />
                </div>
